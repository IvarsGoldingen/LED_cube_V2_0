...
class Max9841 {
    private:
        ...
        //Gain currently set on the ic chip
        byte currentGain = 0xff;

    public:
        //the three gains settable on the Max9841. See void setGain(byte gain);
        const static byte GAIN_LOW = 0;
        const static byte GAIN_MEDIUM = 1;
        const static byte GAIN_HIGH = 2;
        /***
         * Constuctor
         ...
         */
        void setGain(byte gain);
        //Get the gain set on the Microphone object
        byte getMicGain();
        /***
         * getSoundLevel();
